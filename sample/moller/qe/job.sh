...
function task_scf () {
  ...
  [ $_debug -eq 1 ] && echo "DEBUG: $_work_item: srun --exclusive --mem-per-cpu=1840 -N $_nn -n $_np -c $_nc pw.x -in scf.in > scf.out"
  srun --exclusive --mem-per-cpu=1840 -N $_nn -n $_np -c $_nc pw.x -in scf.in > scf.out
}
export -f task_scf

date
echo "  scf"
printf '%s\n' ${mplist[@]} | run_parallel 0,8,16 task_scf log_scf.dat
...
function task_nscf () {
  ...
  [ $_debug -eq 1 ] && echo "DEBUG: $_work_item: srun --exclusive --mem-per-cpu=1840 -N $_nn -n $_np -c $_nc pw.x -in nscf.in > nscf.out"
  srun --exclusive --mem-per-cpu=1840 -N $_nn -n $_np -c $_nc pw.x -in nscf.in > nscf.out
}
export -f task_nscf

date
echo "  nscf"
printf '%s\n' ${mplist[@]} | run_parallel 0,8,16 task_nscf log_nscf.dat
...
function task_band () {
  ...
  [ $_debug -eq 1 ] && echo "DEBUG: $_work_item: srun --exclusive --mem-per-cpu=1840 -N $_nn -n $_np -c $_nc bands.x -in band.in > band.out"
  srun --exclusive --mem-per-cpu=1840 -N $_nn -n $_np -c $_nc bands.x -in band.in > band.out
}
export -f task_band

date
echo "  band"
printf '%s\n' ${mplist[@]} | run_parallel 0,8,16 task_band log_band.dat
