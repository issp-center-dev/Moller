...
function task_task1 () {
  ...
  [ $_debug -eq 1 ] && echo "DEBUG: $_work_item: srun --exclusive --mem-per-cpu=1840 -N $_nn -n $_np -c $_nc python3 ../moller_test.py $_work_item --sleep 3 --fail 10"
  srun --exclusive --mem-per-cpu=1840 -N $_nn -n $_np -c $_nc python3 ../moller_test.py $_work_item --sleep 3 --fail 10
}
export -f task_task1

date
echo "  task1"
printf '%s\n' ${mplist[@]} | run_parallel 0,1,16 task_task1 log_task1.dat
...
function task_task2 () {
  ...
  if ! _is_ready $_work_item log_task1.dat; then
    exit -1
  fi
  cd $_workdir
...
date
echo "  task2"
printf '%s\n' ${mplist[@]} | run_parallel 0,8,16 task_task2 log_task2.dat
...
function task_task3 () {
  ...
  if ! _is_ready $_work_item log_task2.dat; then
    exit -1
  fi
  cd $_workdir
...
date
echo "  task3"
printf '%s\n' ${mplist[@]} | run_parallel 0,12,16 task_task3 log_task3.dat
...
function task_task4 () {
  ...
  if ! _is_ready $_work_item log_task3.dat; then
    exit -1
  fi
  cd $_workdir
...
date
echo "  task4"
printf '%s\n' ${mplist[@]} | run_parallel 2,16,8 task_task4 log_task4.dat
