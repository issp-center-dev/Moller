...
ここでは "parallel = false" に設定しています。この場合、ジョブ単位での並列は行われず逐次的にジョブが実行されます。
次に、"hello world."を出力するタスクを hello worldというタスク名で定義しています。
ここでは "parallel" が設定されていないので、 "paralle = true" として実行されます。この場合、ジョブ単位での並列が行われます。
...
リストに記載されたディレクトリに、それぞれ "result.txt" というファイルが生成されます。
"result.txt" には、ジョブ結果の "hello world.", "hello world again." という文字列が出力されていることが確認できます。
